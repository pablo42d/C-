...
namespace SystemTelefonicznyGY.Logika
{
    public class BilingService
    {
        ...
        // --- 1. METODY IMPORTU (Zachowane ze starej wersji) ---
...
//using System.Linq;
//using System.Text; // Do StringBuilder (budowanie CSV)
//using System.Web; // Do HttpPostedFileBase
//using SystemTelefonicznyGY.Models;
...
//        // Pomocnicza struktura danych zwracana po imporcie
//        public struct WynikImportu
//        {
//            public int LiczbaRekordow;
//            public string NumerFaktury;
...
//                    var wartosci = linia.Split(';');
//                    if (wartosci.Length < 9) continue; // Zabezpieczenie przed błędną linią

//                    // Pobranie nr faktury i ewentualne usunięcie starych danych
//                    string nrFaktury = wartosci[8].Trim().Replace("'", "''"); // Zabezpieczenie przed SQL Injection (escape apostrofu)
...
//                    else
//                    {
//                        // Fallback - próba domyślnego parsowania
//                        DateTime.TryParse(wartosci[0], out dt);
//                        dataDlaSQL = dt.ToString("yyyy-MM-dd HH:mm:ss");
//                    }

//                    // Przygotowanie danych (zamiana przecinków na kropki dla SQL, czyszczenie numerów)
//                    string numerA = KonwertujNumer(wartosci[1]);
//                    string numerB = KonwertujNumer(wartosci[2]);
//                    string typPol = wartosci[3].Replace("'", "''");
//                    string czas = wartosci[5];
...
//        // Metoda pomocnicza do naprawy formatu naukowego (np. 4,815E+10 -> 48150000000)
//        private string KonwertujNumer(string raw)
//        {
//            if (string.IsNullOrWhiteSpace(raw)) return "";
//            raw = raw.Trim().Replace(" ", "");
...
//        // Metoda generująca SQL do pobierania raportów (używana przez widok i eksport)
//        public DataTable PobierzDaneRaportu(string warunekCzasu, string filtryDodatkowe, int pomin = 0, int rozmiarStrony = 0)
//        {
//            string sqlBazowy = $@"
//                SELECT b.DataPolaczenia, b.NumerTelefonu, b.KwotaNetto, b.KwotaBrutto, b.NrFaktury, 
...
//            {
//                // Paginacja
//                sql = $@"SELECT * FROM ({sqlBazowy}) AS Zbiorcze 
...
//        // Zwraca StringBuildera gotowego do zapisu do pliku
//        public StringBuilder GenerujCsvRaportu(DataTable dt)
//        {
//            StringBuilder sb = new StringBuilder();
...
//        //public int PoliczRekordy(string warunekCzasu, string filtryDodatkowe)
//        //{
//        //    // Uproszczone zapytanie bez JOINów byle szybko policzyć, albo pełne
//        //    // Tutaj użyjemy pełnego wewnątrz podzapytania
//        //    // Uwaga: To duplikuje logikę SQL z PobierzDaneRaportu, można to wydzielić do prywatnej metody zwracającej string
//        //    // Dla uproszczenia tutaj wstawiam "stub" logiczny

//        //    // ... (Logika generowania SQL identyczna jak w PobierzDaneRaportu - część wewnętrzna)
//        //    // Tutaj dla skrótu zakładamy, że metoda wywołująca przekaże SQL lub użyjemy tej samej logiki stringów.
//        //    return 0; // W implementacji przenieś string sqlBazowy do pola statycznego lub metody prywatnej.
//        //}

//        public decimal PobierzSumeKosztow(int miesiac, int rok)
//        {
//            // Sumujemy brutto z komórek i stacjonarnych dla danego miesiąca
//            string sql = $@"
//        SELECT SUM(Kwota) FROM (
//            SELECT KwotaBrutto AS Kwota FROM BilingiKomorkowe WHERE MONTH(DataPolaczenia) = {miesiac} AND YEAR(DataPolaczenia) = {rok}
//            UNION ALL
...
//            // Zabezpieczenie: jeśli nie ma żadnych rozmów (wynik DBNull), zwracamy 0
//            if (dt.Rows.Count > 0 && dt.Rows[0][0] != DBNull.Value)
//            {
//                return Convert.ToDecimal(dt.Rows[0][0]);
//            }
...
//        // Zwraca krotkę (miesiąc, rok) z datą ostatniego dostępnego bilingu dla pracownika
//        public (int m, int r) PobierzDateOstatniegoBilinguPracownika(int idPracownika)
//        {
//            string sql = $@"
//        SELECT TOP 1 YEAR(DataPolaczenia) as R, MONTH(DataPolaczenia) as M 
...
//            // Jeśli brak danych, zwracamy obecną datę
//            return (DateTime.Now.Month, DateTime.Now.Year);
//        }

//        // Pobiera szczegółowy biling (komórkowy lub stacjonarny) dla pracownika za dany okres
//        public DataTable PobierzBilingiPracownika(int idPracownika, int m, int r, string typ)
//        {
//            string sql;
